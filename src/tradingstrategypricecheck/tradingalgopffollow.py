...
class FollowAlgo:
    ...
    def buy_share(self,current):       
        is_buy_status = self.trade_transact_wrapper.buy_share_MIS(current,self.quantity)
        if(is_buy_status):
    ...
    def sell_share(self,current):    
        is_sell_status = self.trade_transact_wrapper.sell_share_MIS(current,self.quantity)
        if(is_sell_status):
    ...
    def track_buy(self,bought_price,stop_loss,purchase_diff,share_name):    
        ...
        while True:          
            ...
            if(current_price_int < stop_loss):
                is_sold = self.sell_share(current_price_int)
    ...
    def track_sell(self,sold_price,stop_loss,purchase_diff,share_name):
        current_price_First = self.get_share_price(share_name)  
        is_bought = False
        ...
        while True:
            ...
            if(stop_loss_2 < stop_loss):
                ...
                self.logger.info("Current Shre Price  : " + str(current_price_int))
            if(current_price_int > stop_loss):
                is_bought = self.buy_share(current_price_int)
            elif(current_price_int < test_profit_fifty_paise):
                is_bought = self.buy_share(current_price_int)
    ...
    def tradealgostart(self,purchase_diff,share_name):
        ...
        while True:
            ...
            if(result == "Bull"):  
                ...
                buy_status = self.buy_share(current_price_int)                
                ...
                self.track_buy(bought_price,stop_loss,purchase_diff,share_name)
                time.sleep(10)
            ...
            if(result == "Bear"):
                ...
                sell_status = self.sell_share(current_price_int)
                ...
                self.track_sell(sold_price,stop_loss,purchase_diff,share_name)
                time.sleep(10)
