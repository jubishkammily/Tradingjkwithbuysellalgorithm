...
class KiteLoginUtil:
    ...
    def __init__(self,logging=None,exchange="NSE"):
        self.index=0
        if logging == None:
    ...
    def read_credentials(self):
        ...
        try:
            self.username=self.decode(keyring.get_password("zerodha", "username"))
            self.password=self.decode(keyring.get_password("zerodha", "password"))
            self.pin=api_key=self.decode(keyring.get_password("zerodha", "pin"))
            self.api_key=keyring.get_password("zerodha", "api_key")
            self.api_secret=keyring.get_password("zerodha", "api_secret")
            self.access_token=keyring.get_password("zerodha", "access_token")
            self.logging.info('Completed Reading credentials from keys api_key {}\n access_token {}'.format(self.api_key,self.access_token))
    ...
    def login(self,show=True):
        ...
        try:
            if(len(self.access_token.strip())==0):
                self.logging.debug("access_token length is 0 triggering selenium login")
                self.login_new_session()
            self.kite = KiteConnect(api_key=self.api_key)
            self.logging.debug("setting access token")
            self.kite.set_access_token(self.access_token)
            self.validate_lt_read()
            self.logging.debug("logged in with existing access_token")
        except Exception as e:
    ...
    def login_new_session(self,show=True):
        self.logging.debug("invoking login_new_session")
        try:
            self.kite = KiteConnect(api_key=self.api_key)
            url=self.kite.login_url()
            self.logging.debug("Login URL:"+str(url))

            login_util=SeleniumUtil(url,self.username,self.password,self.pin,show,self.logging)
            request_token=login_util.get_request_token()
            #request_token="PROVIDE_REQUEST_TOKEN"

            self.logging.debug("Request Token:"+str(request_token))
            data = self.kite.generate_session(request_token, api_secret=self.api_secret)
            self.access_token=data["access_token"]
            self.validate_lt_read()
            self.save_access_token()
            self.logging.debug("Logged in with new access_token")
        except Exception as e:
            self.logging.critical('failed to connect or read from to Kite with fresh login, Pls try later :')
            self.logging.critical(e, exc_info=True)
    ...
    def validate_lt_read(self):
        ltprice=self.ltp("NSE:INFY")
        self.logging.info("Login Success validate with INFY LTP is:"+str(ltprice))
